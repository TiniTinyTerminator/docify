...
impl DocItem {
    ...
    pub fn display_signature(&self) -> String {
        ...
        match (&self.lang, &self.kind) {
            ...
            (DocLanguage::Java | DocLanguage::D, DocKind::Function) => {
            ...
            (DocLanguage::Go, DocKind::Function) => {
                sig_go(&self.signature).unwrap_or_else(|| sig_clean(&self.signature))
            }
...
pub(crate) fn lang_from_ext(ext: &str) -> DocLanguage {
    match ext {
        "c" | "h" => DocLanguage::C,
        "cpp" | "cc" | "cxx" | "c++" | "hpp" | "hh" | "hxx" | "cuh" | "cu" | "hip" | "sycl"
        | "ispc" => DocLanguage::Cpp,
        "rs" => DocLanguage::Rust,
...
public void parse(String s) {}"#;
    ...
    fn go_comment_with_blank_line_ignored() {
        ...
        assert!(
            got.is_empty(),
            "blank line should break doc comment association"
        );
    }
    ...
    fn go_package_qualifies_items() {
        ...
        let got = items(src, &DocLanguage::Go);
        assert!(
            got.iter().any(|i| i.name == "geom.Clamp"),
    ...
    fn go_package_main_qualifies() {
        ...
        let got = items(src, &DocLanguage::Go);
        assert!(
            got.iter().any(|i| i.name == "main.Run"),
